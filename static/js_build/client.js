...
function updateServerDisplay() {
    ...
    if (currentServerElement)
        currentServerElement.textContent = currentServer.name;
    // Update File List
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".file-list tr").forEach(row => {
        row.addEventListener("click", function () {
            var _a, _b;
            const fileName = (_b = (_a = this.cells[0]) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.trim(); // Get file name from row
            if (fileName)
                openPopup(fileName);
        });
    });
...
function openPopup(fileName) {
    ...
    if (!popup || !popupTitle || !popupFileContent)
        return;
    ...
    const fileContent = currentServer === null || currentServer === void 0 ? void 0 : currentServer.files[fileName];
...
function updateFileList(files) {
    ...
    if (Object.keys(files).length !== 0) {
        ...
        for (const [fileName, key] of Object.entries(files)) {
            ...
            row.innerHTML = `
               <td>${fileName}</td>
               <td style="text-align:right;">${((_b = file_lists[get_server_id_from_current_server_index()]) === null || _b === void 0 ? void 0 : _b.name) || ''}</td>
            `;
...
function updateFile(file_content) {
    ...
    if (!fileNameElement)
        return;
    const fileName = (_a = fileNameElement.textContent) === null || _a === void 0 ? void 0 : _a.trim();
    if (!fileName)
        return;
    ...
    if (extension === "html") {
        ...
        const processedContent = parts.map(part => {
            ...
            if (mediaMatch) {
                ...
                else if (!((_a = win.requestedMedia) === null || _a === void 0 ? void 0 : _a.has(reference))) {
                    // Request the media only if it hasn't been requested yet.
                    if (!win.requestedMedia)
                        win.requestedMedia = new Set();
